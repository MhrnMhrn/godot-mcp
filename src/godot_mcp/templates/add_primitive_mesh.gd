...
func _init() -> void:
    ...
    var mesh_type = str(args.get("mesh-type", "")).strip_edges()
    ...
    if mesh_type.is_empty():
        printerr("Missing required argument: --mesh-type")
        quit(1)
        return
    ...
    var parent_node = root if parent_path.is_empty() or parent_path == "." else root.get_node_or_null(parent_path)
    if parent_node == null or not (parent_node is Node):
    ...
    if raw_config == null or not (raw_config is Dictionary):
        printerr("Primitive mesh config file must contain a JSON object.")
        quit(1)
        return
    var config: Dictionary = raw_config

    if not ClassDB.class_exists(mesh_type):
        printerr("Unknown Godot class: %s" % mesh_type)
        quit(1)
        return
    if not ClassDB.can_instantiate(mesh_type):
        printerr("Class cannot be instantiated: %s" % mesh_type)
        quit(1)
        return

    var mesh_instance_value: Variant = MeshInstance3D.new()
    if mesh_instance_value == null or not (mesh_instance_value is MeshInstance3D):
        printerr("Could not instantiate MeshInstance3D.")
        quit(1)
        return
    var mesh_instance: MeshInstance3D = mesh_instance_value
    mesh_instance.name = node_name

    var mesh_value: Variant = ClassDB.instantiate(mesh_type)
    if mesh_value == null or not (mesh_value is PrimitiveMesh):
        printerr("Class is not a PrimitiveMesh: %s" % mesh_type)
        quit(1)
        return
    var mesh: PrimitiveMesh = mesh_value
    mesh.resource_local_to_scene = true

    var mesh_parameters_raw: Variant = config.get("mesh_parameters", {})
    if mesh_parameters_raw == null or not (mesh_parameters_raw is Dictionary):
        printerr("`mesh_parameters` must be a JSON object when provided.")
        quit(1)
        return
    var mesh_parameters: Dictionary = mesh_parameters_raw

    var transform_raw: Variant = config.get("transform", {})
    if transform_raw == null or not (transform_raw is Dictionary):
        printerr("`transform` must be a JSON object when provided.")
        quit(1)
        return
    var transform: Dictionary = transform_raw

    var updated_mesh_parameters = _apply_mesh_parameter_updates(mesh, mesh_parameters)
    if not transform.is_empty():
        _apply_node3d_transform_updates(mesh_instance, transform)
    ...
    var mesh_details = _collect_mesh_details(mesh)
    print(JSON.stringify({
        ...
        "transform": _serialize_node3d_transform(mesh_instance),
    }))
    quit()
...
func _apply_node3d_transform_updates(node: Node3D, updates: Dictionary) -> void:
    var supported_fields = ["position", "rotation", "rotation_degrees", "scale"]
    for raw_key in updates.keys():
        var field_name = str(raw_key)
        if not supported_fields.has(field_name):
            printerr("Unsupported Node3D transform field: %s" % field_name)
            quit(1)
            return

    if updates.has("position"):
        node.position = _merge_vector3(node.position, updates["position"], "position")
    if updates.has("rotation"):
        node.rotation = _merge_vector3(node.rotation, updates["rotation"], "rotation")
    if updates.has("rotation_degrees"):
        node.rotation_degrees = _merge_vector3(node.rotation_degrees, updates["rotation_degrees"], "rotation_degrees")
    if updates.has("scale"):
        node.scale = _merge_vector3(node.scale, updates["scale"], "scale")


func _serialize_node3d_transform(node: Node3D) -> Dictionary:
    return {
        "position": _vector3_to_dict(node.position),
        "rotation": _vector3_to_dict(node.rotation),
        "rotation_degrees": _vector3_to_dict(node.rotation_degrees),
        "scale": _vector3_to_dict(node.scale),
    }
